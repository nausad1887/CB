...
@Component({
  selector: 'app-shared-selected',
  templateUrl: './shared-selected.component.html',
  styleUrls: ['./shared-selected.component.css'],
})
export class SharedSelectedComponent implements OnInit {
  ...
  public onDeclineClick = (interviewID: string) => {
    this.declineSelectedJob.emit(interviewID);
  }
  public onAcceptClick = (interviewID: string) => {
    this.acceptSelectedJob.emit(interviewID);
  }
}
